...
class BaseSolver:
    ...
    def build_optimizer(self):
		...
		# dict({'FC1': opt.TRAIN.LR_MULT,'FC2': opt.TRAIN.LR_MULT}))

        self.optimizer = {}

        param_groups_G_1 = [{'params': self.net_1.module.feature_extractor.parameters(), 'lr_mult': 1.0}]
        param_groups_FC_1 = [{'params': self.net_1.module.FC.parameters(), 'lr_mult': opt.TRAIN.LR_MULT}]

        param_groups_G_2 = [{'params': self.net_2.module.feature_extractor.parameters(), 'lr_mult': 1.0}]
        param_groups_FC_2 = [{'params': self.net_2.module.FC.parameters(), 'lr_mult': opt.TRAIN.LR_MULT}]

        assert opt.TRAIN.OPTIMIZER in ["Adam", "SGD"], \
            "Currently do not support your specified optimizer."

        if opt.TRAIN.OPTIMIZER == "Adam":
            self.optimizer['G_1'] = optim.Adam(param_groups_G_1, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC_1'] = optim.Adam(param_groups_FC_1, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['G_2'] = optim.Adam(param_groups_G_2, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC_2'] = optim.Adam(param_groups_FC_2, 
			lr=self.base_lr, betas=[opt.ADAM.BETA1, opt.ADAM.BETA2], 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
        elif opt.TRAIN.OPTIMIZER == "SGD":
            self.optimizer['G_1'] = optim.SGD(param_groups_G_1, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC_1'] = optim.SGD(param_groups_FC_1, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['G_2'] = optim.SGD(param_groups_G_2, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
            self.optimizer['FC_2'] = optim.SGD(param_groups_FC_2, 
			lr=self.base_lr, momentum=self.momentum, 
			weight_decay=opt.TRAIN.WEIGHT_DECAY)
    ...
    def test(self):
        ...
        for sample in iter(self.test_data['loader']):
            data_1, data_2, gt = to_cuda(sample['Img_1']), to_cuda(sample['Img_2']), to_cuda(sample['Label'])
            logits = (self.net_1(data_1)['logits'] + self.net_2(data_2)['logits'])/2
            preds += [logits]
            gts += [gt]
